...
write_struct_field_test :: proc(t: ^testing.T) {
    S :: struct($T: typeid) {
        padding1: [13]u8 `duckdb:"Padding1"`,
        value:    T `duckdb:"Value"`,
        padding2: [5]u8 `duckdb:"Padding2"`,
    }
...
get_type_fields :: proc($T: typeid) -> ([]Field_Type, Field_Error) {
    ...
    for i in 0 ..< reflect.struct_field_count(T) {
        ...
        capture, err := match_and_return_capture(`duckdb:"(.*?)"`, cast(string)field.tag)
        if err != nil {
            defer delete_field_types(out[:])

            return nil, fmt.tprintf(
                "could not find duckdb tag on field '{}' of struct '{}'. err: {}",
                field.name,
                struct_info,
                err,
            )
