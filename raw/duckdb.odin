...
LINK :: #config(DUCKDB_LINK, "shared")
...
when LINK == "system" {
    ...
    when ODIN_OS == .Darwin {
        when ODIN_ARCH == .amd64 {
            when LINK == "shared" {
                LIB_PATH :: "../darwin_x64/libduckdb.dylib"
            } else when LINK == "static" {
                LIB_PATH :: "../darwin_x64/libduckdb_static.darwin.a"
            }
        } else when ODIN_ARCH == .arm64 {
            when LINK == "shared" {
                LIB_PATH :: "../darwin_arm64/libduckdb.dylib"
            } else when LINK == "static" {
                LIB_PATH :: "../darwin_arm64/libduckdb_static.darwin.a"
            }
        } else {
            #panic("vendor/duckdb supports Darwin amd64 and arm64 only")
        }
    } else when ODIN_OS == .Linux {
        when ODIN_ARCH == .amd64 {
            when LINK == "shared" {
                LIB_PATH :: "../linux_x64/libduckdb.so"
            } else when LINK == "static" {
                LIB_PATH :: "../linux_x64/libduckdb_static.linux.a"
            }
        } else when ODIN_ARCH == .arm64 {
            when LINK == "shared" {
                LIB_PATH :: "../linux_arm64/libduckdb.so"
            } else when LINK == "static" {
                LIB_PATH :: "../linux_arm64/libduckdb_static.linux.a"
            }
        } else {
            #panic("vendor/duckdb supports Linux amd64 and arm64 only")
        }
    } else when ODIN_OS == .Windows {
        when ODIN_ARCH == .amd64 {
            when LINK == "shared" {
                LIB_PATH :: "../windows_x64/duckdb_shared.lib"
            } else when LINK == "static" {
                LIB_PATH :: "../windows_x64/duckdb.lib"
            }
        } else when ODIN_ARCH == .arm64 {
            when LINK == "shared" {
                LIB_PATH :: "../windows_arm64/duckdb_shared.lib"
            } else when LINK == "static" {
                LIB_PATH :: "../windows_arm64/duckdb.lib"
            }
        } else {
            #panic("vendor/duckdb supports Windows amd64 and arm64 only")
        }
    } else {
        #panic("vendor/duckdb supports Windows, Darwin, Linux, and system linking only")
    }
    foreign import duckdb {LIB_PATH}
}
...
duckdb_pending_state :: enum c.int {
...
duckdb_statement_type :: enum c.int {
...
duckdb_table_function_bind_t :: ^proc "c" (info: duckdb_bind_info)

//! The (possibly thread-local) init function of the table function.
duckdb_table_function_init_t :: ^proc "c" (info: duckdb_init_info)

//! The main function of the table function.
duckdb_table_function_t :: ^proc "c" (info: duckdb_function_info, output: duckdb_data_chunk)
...
duckdb_replacement_callback_t :: ^proc "c" (info: duckdb_replacement_scan_info, table_name: cstring, data: rawptr)
